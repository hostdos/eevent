 ...
 */
class SpiessPreordersController extends AppController {


	public $components = array('Session');
var $uses = array("Preorder");
/**
 * index method
 *
 * @return void
 */
	public function index() {
		...
		$this->paginate = array('conditions' => array('Preorder.user_id' => $userp['id'], "Preorder.drinktype<>''", "Preorder.spiesstype<>''"));
	...
	public function admin_index() {
		$this->Preorder->recursive = 0;
		$this->paginate = array('conditions' => array("Preorder.drinktype<> ''", "Preorder.spiesstype<>''"));
		$this->set('preorders', $this->paginate());
		$this->set('sum', $this->Preorder->query("SELECT  `spiesstype` , SUM( spiesstype ) 
FROM  `preorders` 
WHERE  `spiesstype` <>  ''
AND  `drinktype` <>  ''
GROUP BY  `spiesstype`"));
 ...
 */
	...
	private function checkPaid($id = null){
		...
		if($userreg['Registrations']['paid'] != 1 && $userreg['Registrations']['checkin'] != 1){
			$this->Session->setFlash(__('Leider hast du noch nicht eingecheckt, deshalb kannst du keine Energy-Drinks vorbestellen.'));
 ...
 */
	public function add() {
		if ($this->request->is('post')) {
			...
			$this->checkPaid();
			if($this->request->data['Preorder']['amount'] > 0){
