...
public $helpers = array('Html');
	...
	public function getNewsSingle($id){
		...
		/*
		 * Laden und auslesen allre benötigten Daten.
		 */
		...
		// output definieren und HTML Generieren
		$_output = null;
	...
	public function getAdminNewsSingle($id){
		...
		/*
		 * Laden und Auslesen aller Daten
		 */
	...
	public function getNewsBig($count){
		...
		if($commentnum == 1){$_output .= $this->Html->link( $commentnum .' '. __('Kommentar'),
			array('controller' => 'news', 'action' => 'view',$nws['News']['id']), array('class' => 'news-comments')); 
		}else{
		$_output .= $this->Html->link( $commentnum .' '. __('Kommentare'), array('controller' => 'news', 'action' => 'view'
	...
	public function getAdminNewsBig($count){
		...
		foreach ($News as $nws) {
			...
			if($commentnum == 1)
			{
				$_output .= $this->Html->link( $commentnum .' '. __('Kommentar'), array('controller' => 'news', 'action' => 'view'
				,$nws['News']['id']), array('class' => 'news-comments')); 
			}else{
				$_output .= $this->Html->link( $commentnum .' '. __('Kommentare'), array('controller' => 'news', 'action' => 'view'
				,$nws['News']['id']), array('class' => 'news-comments')); 
